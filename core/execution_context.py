    ...
    def resolve(self, template: str | Any) -> Any:
        ...
        if resolved != template:
            ...
            try:
                ...
                # 还原占位符为原始值
                return _restore_placeholders(parsed)
...
_placeholder_counter = itertools.count(start=1)
_value_placeholder_map: dict[str, Any] = {}


def _json_dumps_placeholder(value: Any) -> str:
    """为非 JSON 兼容类型生成唯一占位符，并存储值到映射表。"""
    key = f"__JSON_VALUE_{next(_placeholder_counter)}__"
    _value_placeholder_map[key] = value
    return f'"{key}"'
...
def _restore_placeholders(obj: Any) -> Any:
    ...
    if isinstance(obj, dict):
        ...
        return [_restore_placeholders(item) for item in obj]
    elif isinstance(obj, str) and obj in _value_placeholder_map:
        return _value_placeholder_map[obj]
    return obj
